...
- Go 1.22+
- [Tart](https://tart.run) on PATH — setup: [tart.md](./tart.md)
- [softnet](https://github.com/cirruslabs/softnet) on PATH when host proxy is on (default). Setup: [softnet.md](./softnet.md). Skip with `network.proxy.disabled: true`.
...
`cage vm start` applies resolved **mounts** (`tart run --dir`) and **copies** (into the guest after the VM is ready). Paths on `fs.deny` fail at `LoadResolved` / inspect — they never reach Tart.

By default start also runs a host **HTTP CONNECT** proxy (MITM on) plus SOCKS, and softnet host-only so the guest cannot bypass ([softnet setup](./softnet.md)). Allowlists live on `network.plugins.egress`. Opt out with `network.proxy.disabled: true`. Set `network.proxy.logging: true` for CONNECT JSONL at `.cage/run/<id>/proxy.log` — follow with `cage vm logs -f` or `cage vm start -f`. Install egress when using an allowlist: `cage plugin install -l ./plugins/network/egress`. Guest proxy env uses `http://` (Node-safe): `/var/lib/cage/proxy.env`, profile hooks, `/var/lib/cage/shell`, and MITM CA install. Agents call real `https://…` URLs; Cage injects via `http-proxy` Host match. Certificate pinning may still break some clients.
